...
## Тесты
Для базового функционала (`Сервис блюд` и `Сервис приемов пищи`) были написаны тесты, которые покрывает 100% сценариев работы с этими сервисами.

Выполнение тестов для сервиса блюд
<br>![screenshot](images/dish-service-test.png)

Выполнение тестов для сервиса приемов пищи
<br>![screenshot](images/meal-service-test.png)

> [!NOTE]  
> Тесты запускаются при каждом запуске приложения через `docker-compose.yaml`
...
> [!WARNING]
> Переменные окружения базы данных хранятся в файле `.env.example`, при деплое сервиса в прод необходимо заменить название файла на `.env` и добавить его в `.gitignore` во избежание нарушения конфиденциальности данных
